...
%  \node [box-rect, right=0.3 of Highthrough-Put, fill=white, text width=0.5em, minimum height=6.0em, yshift=-2.1em, very thin] (Modeling-Initial) {\fontsize{3.0pt}{2.5pt}\selectfont{ 结\\\vskip -8.0pt构\\\vskip -8.0pt建\\\vskip -8.0pt模\\\vskip -8.0pt与\\\vskip -8.0pt初\\\vskip -8.0pt始\\\vskip -8.0pt化}};
...
%  \node [box-rect, right=0.8 of Process-Manage, fill=red!50, text width=4.5em, minimum height=3.0em] (Multi-Scale) {\fontsize{3.0pt}{2.5pt}\selectfont{\vskip -10.0pt高性能计算任务\\\textrm{DFT-MD-$\cdots$}\\\vskip 1pt \textrm{(跨尺度计算)}\\\vskip -8.0pt\textrm{+}集成核心计算软件接口}};
%  \node [box-rect, right=0.65 of Process-Manage, fill=orange!50, text width=4.2em, minimum height=0.2em, yshift=-0.8em] (Custodian) {\fontsize{3.0pt}{2.5pt}\selectfont{容错机制\textrm{Cudodian}模块}};
%  \node [box-rect, below=0.05 of Modeling-Initial, fill=white, text width=0.5em, minimum height=5.6em, very thin] (Multi-Scale-Simulation) {\fontsize{3.0pt}{2.5pt}\selectfont{多\\\vskip -8.0pt尺\\\vskip -8.0pt度\\\vskip -8.0pt计\\\vskip -8.0pt算\\\vskip -8.0pt与\\\vskip -8.0pt模\\\vskip -8.0pt拟}};
...
%
%  \node [box-rect, below=0.5 of DataBase, fill=olive!60, text width=5.5em, minimum height=1.5em] (Data-Manage) {\fontsize{3.0pt}{2.5pt}\selectfont{计算结果数据与管理}};
%  \node [box-rect, below=0.6 of Data-Manage, fill=lime!70, text width=3.5em, minimum height=1.5em, xshift=-0.5em] (Result-Analysis) {\fontsize{3.0pt}{2.5pt}\selectfont{计算结果可视化分析}};
...
%  \node [box-rect, below=1.5 of Data-Manage, fill=darkgray!70, text width=10.5em, minimum height=1.5em, xshift=4.0em] (Application) {\fontsize{3.0pt}{2.5pt}\selectfont{\textcolor{white}{特色应用场景:~原子间多体相互作用}}};
%  \node [box-rect, below=0.05 of Multi-Scale-Simulation, fill=white, text width=0.5em, minimum height=6.5em, very thin] (Result-Machine-Learning) {\fontsize{3.0pt}{2.5pt}\selectfont{计\\\vskip -8.0pt算\\\vskip -8.0pt结\\\vskip -8.0pt果\\\vskip -8.0pt分\\\vskip -8.0pt析\\\vskip -8.0pt与\\\vskip -8.0pt机\\\vskip -8.0pt器\\\vskip -8.0pt学\\\vskip -8.0pt习}};
%
%%%%%%% ------------ 定义 虚线方框转角 -----------------  %%%%%%
%\path
...
%  (Process-Manage.north) ++(-2.5em, 0.3em) coordinate (DataBase-1)
%  (Data-Manage.south) ++ (2.5em, -0.1em) coordinate (DataBase-2)
%  (Process-Design.south)++(-2.0em, -1.50em) coordinate (Process-Design-1)
%  (Process-Design.south)++(-8.5em, -1.50em) coordinate (Process-Design-2)
...
%  (Application.north)++(-4.51em, 0.0em) coordinate(Application-1)
...
%\draw [gray, dotted, line width=2pt] (DataBase.south)++(-3.5em,-0.78em) -- (Multi-Scale-Simulation.south);
%%%%%%% ------------ 绘制 虚线方框 -----------------  %%%%%%
