...
\frame
{
	...
	\fontsize{8.5pt}{6.2pt}\selectfont{\begin{displaymath}
		\begin{aligned}
			...
			&W(\mathbf{1},\mathbf{2})=v(\mathbf{1},\mathbf{2})+\int\mathrm{d}(\mathbf{34})v(\mathbf{1},\mathbf{3})P(\mathbf{3},\mathbf{4})W(\mathbf{4},\mathbf{2})
		\end{aligned}
	\end{displaymath}}
}
...
\frame
{
	...
	\fontsize{7.5pt}{6.2pt}\selectfont{\begin{displaymath}
		\begin{aligned}
			P_{\vec G_1,\vec G_2}^0(\vec q,\omega)=&\sum_{\mathrm{spin}}\sum_{\vec k}\sum_n^{\mathrm{occ}}\sum_m^{\mathrm{unocc}}\langle\vec k,n|\mathrm{e}^{\mathrm{i}(\vec q+\vec G_1)\cdot\vec r}|\vec k+\vec q,m\rangle\langle\vec k+\vec q,m|\mathrm{e}^{-\mathrm{i}(\vec q+\vec G_2)\cdot\vec r}|\vec k,n\rangle\\
...
\frame
{
	...
	\fontsize{8.5pt}{6.2pt}\selectfont{
		...
		\hspace*{-10pt}
		\begin{aligned}
			&\epsilon_{\vec G\vec G^{\prime}}(\vec q,\omega)=\delta_{\vec G\vec G^{\prime}}+\frac{4\pi}{|\vec q+\vec G||\vec q+\vec G^{\prime}|}\alpha_{\vec G\vec G^{\prime}}^0(\vec q,\omega)\\
			&P_{\vec G\vec G^{\prime}}^0(\vec q,\omega)=\sum_{\mathrm{spin}}\sum_{\vec k}\sum_n^{\mathrm{occ}}\sum_m^{\mathrm{unocc}}\frac{\langle\vec k,n|\mathrm{e}^{\mathrm{i}(\vec q+\vec G_1)\cdot\vec r}|\vec k+\vec q,m\rangle\langle\vec k+\vec q,m|\mathrm{e}^{-\mathrm{i}(\vec q+\vec G_2)\cdot\vec r}|\vec k,n\rangle}{|\omega-\varepsilon_{\vec k,n}+\varepsilon_{vec k+\vec q,m}}
		\end{aligned}
	\end{displaymath}}
	倒反静态介电矩阵用本征表示
	\begin{displaymath}
...
%\section{\rm{LDA}+$U$与\rm{GWA}}
%\frame{
%	\frametitle{\textrm{LDA}+$U$与\textrm{GWA}}
%}
%\frame
%{
...
%	\bibitem{Xie_Lu}谢希德、陆栋\:主编, {\textit{固体能带理论}}\:复旦大学出版社, 上海, 1998
	...
	\bibitem{RPP61-237_1998}\textrm{F. Aryasetiawan and O. Gunnarsson \textit{Rep. Prog. Phys.}, \textbf{61} (1998), 237}
