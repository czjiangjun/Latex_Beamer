...
%		    \begin{enumerate}
%			    \item {\fontsize{8.5pt}{5.2pt}\selectfont{微观粒子可以有确定的位置和动量，因此可以用明确的径迹(\textrm{trajectory})描述其运动\\
%				    \textcolor{blue}{但对于粒子位置和速度的测量，依然必须遵守不确定性原理}}}
%			    \item {\fontsize{8.5pt}{5.2pt}\selectfont{粒子接受波函数的引导，并通过与量子势\textrm{(Quantum potential)}的交互作用，表现出非局域的整体性}}
