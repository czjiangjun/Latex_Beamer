...
\frame
{
	...
	\begin{displaymath}
		\tilde{V}^{\mathrm{PK}}=V(\vec r)\textcolor{blue}{+\sum_c(E_v-E_c)|\varphi_c\rangle\langle\varphi_c|}
	\end{displaymath}
	对应的赝波函数可以表示为
	\begin{displaymath}
...
\frame
{
	\frametitle{第一原理赝势}
		由第一原理求解出全电子波函数(径向部分)$P_{n,l}(r)$
			...
			$$V_{\mathrm{src},l}^{\mathrm{PP}}(r)=\varepsilon_l\textcolor{red}{-\dfrac{l(l+2)}{2r^2}}+\dfrac{1}{2P_l^{\mathrm{PS}}(r)}\dfrac{\mathrm{d}^2}{\mathrm{d}r^2}P_l^{\mathrm{AE}}(r),\quad r>r_{cl}$$
}
...
\vskip 8pt
\textrm{Kleinman-Bylander}等注意到:~如果角动量$l$相关的半局域型赝势改写成\textrm{PK}型赝势，则平面波展开可写成
\begin{displaymath}
...
\end{displaymath}
显然，大量积分将以乘积形式出现，因此需要独立计算的积分数量将大大减少
}
...
%	引入分离算符$\delta V^{\mathrm{NL}}$满足
	...
	这种分解称为\textrm{factored pseudo-potential}
	\begin{displaymath}
