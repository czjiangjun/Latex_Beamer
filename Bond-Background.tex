...
%	\vskip 5 pt \textrm{2025.02.25}
%	\vskip 5 pt \textrm{2025.05.22}
%	\vskip 5 pt \textrm{2025.08.15}
	\vskip 5 pt \textrm{2025.11.14}
}}
}
...
\frame
{
	\frametitle{分子间相互作用的复杂性}
\begin{figure}[h!]
\centering
\label{MO-Bond_charge-density}
\vspace{-2.7pt}
%\caption{\textrm{The wave-particle duality and Photoelectric effect}}
\includegraphics[height=2.60in,width=4.00in,viewport=0 0 205 120,clip]{Figures/Illustration-of-elements-used-in-the-construction-of-force-fields-The-bonded-terms.png}
\end{figure}
}
...
\frame
{
	\frametitle{原子间相互作用函数}
\begin{figure}[h!]
\centering
\label{MO-Bond_function}
\vspace{-3.5pt}
%\caption{\textrm{The wave-particle duality and Photoelectric effect}}
...
\frame[allowframebreaks]
{
	...
	\begin{itemize}
		\item \textrm{Lennard-Jones}势
\begin{figure}[h!]
\centering
\vskip -0.35in
%\includegraphics[height=1.00in,width=1.35in,viewport=0 0 340 270,clip]{Figures/Lennard-Jones_potential.png}
\includegraphics[height=1.00in,width=1.40in,viewport=0 0 600 430,clip]{Figures/H2-bonding.jpg}\\
...
\end{figure}
	...
	\end{displaymath}
	{\fontsize{7.0pt}{6.2pt}\selectfont{这里$\varepsilon$和$\sigma$是和原子有关的参数
...
\centering
\vspace*{-0.15in}
\includegraphics[height=1.25in,width=1.10in,viewport=0 0 323 420,clip]{Figures/Bond_stretching_energy.png}
...
\frame
{
	\frametitle{化学反应、化学键与势能面}
\begin{figure}[h!]
\centering
\label{MO_E-tot-Vs-Position-Force}
\vspace{-3.5pt}
%\caption{\textrm{The wave-particle duality and Photoelectric effect}}
\includegraphics[height=2.62in,width=4.00in,viewport=0 0 330 242,clip]{Figures/E-V-Force.jpg}
\end{figure}
{\centering\textcolor{magenta}{对化学键本质的认知:~时代呼唤量子力学}}
}
...
\vspace{-10.5pt}
%\caption{\textrm{The wave-particle duality and Photoelectric effect}}
\includegraphics[height=2.90in,width=3.2in,viewport=0 0 870 780,clip]{Figures/Molecular_electrostatic_potential_(MEP)-surface-and-ﬁtting-point_charges-to-the-electrostatic_potential-of-N-(2_bromoethyl)_phthalimide.png}
...
{\fontsize{6.0pt}{4.2pt}\selectfont{
	\textrm{QM/MM}组合的\textrm{MM}部分\textrm{Hamiltonian}($\mathbf{H}$)的相互作用\\
	(\textcolor{magenta}{通常包括与\textrm{QM/MM}边界一分为二的共价键部分})
\begin{itemize}
	\item 共价键的成键作用(即拉伸、弯曲和扭转的贡献)
...
\begin{frame}[allowframebreaks]
	...
	\begin{itemize}
		\item 扣减法\textrm{(subtractive scheme)}:
			...
			\end{displaymath}
			{\fontsize{8.0pt}{4.2pt}\selectfont{其中$E^{\mathrm{MM}}(\mathrm{QM})$是用分子力学方法计算的\textrm{QM}区能量}}\\
		...
		\item 高精度方法:
			\begin{displaymath}
				...
				\end{aligned}
			\end{displaymath}
			{\fontsize{8.0pt}{4.2pt}\selectfont{下标$I$表示\textrm{QM}区的原子核，而下标$I^{\prime}$表示\textrm{MM}区的原子核}}
			\begin{enumerate}
				{\fontsize{7.0pt}{4.2pt}\selectfont{
				\item \textcolor{magenta}{前两项}表示\textrm{QM}区\underline{电荷密度}(源于电子与离子电荷)与\textrm{MM}区\underline{经典电荷}的总的相互作用
				\item \textcolor{violet}{第三项}表示在\textrm{QM/MM}区域的色散相互作用
				\item \textcolor{cyan}{第四项}表示在\textrm{QM/MM}区共价键的伸缩振动势函数
				\item \textcolor{brown}{最后两项}表示\textrm{QM/MM}区共价键的弯曲和旋转振动势函数}}\\
					{\fontsize{6.0pt}{4.2pt}\selectfont{\textcolor{red}{$\theta$和$\phi$由\textrm{QM}与\textrm{MM}区域原子构成的夹角%(极化角和方位角)
				}}}
			\end{enumerate}
	\end{itemize}
\end{frame}
...
\frame
{
	...
	\begin{itemize}
		\item %\textrm{Materials Visualizer}：
			提供搭建分子、晶体及高分子材料\textcolor{blue}{结构模型搭建} %所需的所有工具，是Materials Studio产品系列的核心模块。
		\item %\textrm{COMPASS}：
			支持对凝聚态材料进行原子水平模拟的\textcolor{red}{多种力场}，能在广泛的温度、压力范围内预测孤立体系或凝聚态体系中各种分子的结构、构象、振动及热物理性质
		\item %\textrm{Amorphous Cell}：
...
%		\item %\textrm{DMol3}：
%			密度泛函量子力学程序，可以模拟气相、溶液、表面及固体等过程及性质
%
%		\item %\textrm{CASTEP}：
%			量子力学程序，广泛应用于陶瓷、半导体、金属等多种材料的研究，包括晶体材料的性质、表面和表面重构的性质等
	\end{itemize}
%应用领域
%	\begin{itemize}
%		\item 材料科学研究\\
%			用于构建、显示和分析分子、固体及表面的结构模型，研究、预测材料的相关性质。
%
%		\item 工业应用\\
%			广泛应用于石化、化工、制药、食品、石油、电子、汽车和航空航天等工业部门。
%	\end{itemize}

%优势特点

%用户友好界面：采用\textrm{Microsoft}标准用户界面，方便用户通过各种控制面板直接对计算参数和计算结果进行设置和分析。

%高度模块化：用户可以自由定制、购买自己的软件系统，以满足研究工作的不同需要。

%跨平台兼容：支持多种操作系统，使得用户可以在不同的计算环境中使用\textrm{Materials~Studio}进行研究。

	\textrm{Materials~Studio}是一个功能强大的材料计算软件，它不仅提供了丰富的模块来满足不同研究领域的需求，还具有高度的用户友好性和灵活性%。无论是学术研究还是工业应用，\textrm{Materials~Studio}都能提供强有力的支持。
}
