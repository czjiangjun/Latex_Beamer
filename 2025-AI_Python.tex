...
\begin{frame}
    ...
    \begin{itemize}
        \item 使用Homebrew安装（推荐）
            \begin{itemize}
                \item 若未安装Homebrew，先在终端运行命令%\verb|/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"|安装Homebrew
                \item 安装完成后，运行命令%\verb|brew install python|安装Python
        ...
        \item 使用官方安装包安装
            \begin{itemize}
                ...
                \item 安装完成后，在终端中可通过%\verb|python3 --version|查看Python版本
            \end{itemize}
    \end{itemize}
\end{frame}
...
\end{frame}
%\begin{frame}
%	\frametitle{\textrm{Python}安装与配置环境}
%\textrm{Python}的下载\\
%			{\fontsize{7.2pt}{4.2pt}\selectfont{官方网站:~\url{https:python.org}}}
%	\begin{itemize}
%		\item \textrm{Windows}:\\
%			访问Python官方网站（https://www.python.org/downloads/windows/），下载最新版本的Windows安装程序。
%2. 运行下载的安装程序，在安装向导中勾选“Add Python to PATH”选项，这一步非常关键，因为它会将Python添加到系统的环境变量中，使得你可以在任何地方通过命令行启动Python。
%3. 点击“Install Now”，按照提示完成安装过程。
%		\item \textrm{Python}
%	\end{itemize}
%\end{frame}
