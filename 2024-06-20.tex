...
%  \node [box, draw=blue, fill=white, text width=5.0em, minimum height=1em] at(-3.0,-2.7) (k-space) {\fontsize{5.0pt}{2.5pt}\selectfont{$\vec k$-空间积分方案}};
  ...
  \node [box, draw=blue, below=2.25 of Exch-Corr, fill=purple!30, text width=5.0em, minimum height=0.5em] (k-space) {\fontsize{5.0pt}{2.5pt}\selectfont{$\vec k$-空间积分方案}};
...
\path
  ...
  \node [box, below=0.2 of Criterion, fill=purple!80, text width=7.0em, minimum height=0.5em] (SCF-output) {\fontsize{4.7pt}{1.5pt}\selectfont{输出电荷密度$\rho(\vec r)$\vskip -0.2em 本征态波函数$\Psi_i(\vec r)$ \vskip -1.5em 能量本征值$\varepsilon_i$}};
%  \node [box,below=0.5 of potential] (hamiltonian) {$\hat{H}_{KS}=-\frac{\hbar^2}{2m}\vec{\nabla}^2 + v_{\text{ext},s}(\vec r)$};
%  \node [box,below=0.5 of hamiltonian] (se) {$\hat{H}_{KS} \phi_i(\vec r)= E_i \phi_i(\vec r)$};
%  \node [box,below=0.5 of se] (density) {$\rho(\vec r)=\sum_{i=1}^n f_i\,|\phi_i(\vec r_i)|^2$};
%  \node [box,below=0.5 of density] (criterion) {Convergence criterion satisfied?};
...
\path
  (Basis-set.south) ++(0.0em,-6.8em) coordinate (Basis-set-fit)
  (Charge.west) ++(-1.6em,0.0em) coordinate (Charge-fit)
  (Wave-Eigen.west) ++(-1.6em,0.0em) coordinate (Wave-Eigen-fit)
  (Potential.north west) ++(-3.9em,0.20em) coordinate (Potential-fit2)
  (Criterion.west) ++(-8.5em,0.0em) coordinate (Criterion-fit)
  (SCF-output.west) ++(-11.35em,-1.30em) coordinate (SCF-output-fit)
  (Symmetry-OP.west) ++(-5.1em, 0.0em) coordinate (Symmetry-OP-fit2)
  (k-space.east) ++(4.1em, 0.0em) coordinate (k-space-fit) ;
...
%  \node [rectangle,draw=red, fill=gray!30, inner sep=0.1em,fit=(Fock-matrix-fit) (Kohn-Sham-fit), thick] (enclosure1) {};
 ...
 % \path [arrow] (OS) -- (Figure);
  ...
  \node [box, fill=magenta!50, text width=8.5em, draw=pink, minimum height=0.5em, thick] at (-3.8,-0.35) (Figure) {\includegraphics[width=1.05in, height=0.7in,viewport=0 0 1172 863,clip]{Figures/DFT.png}};

  \path [arrow, draw=purple] (SCF-output.east) --(SCF-properties.west);
  \path [draw, dotted, brown] (Symmetry-OP.west) --(Symmetry-OP-fit2);
  \path [arrow, dotted, draw=brown] (Symmetry-OP-fit2) --(SCF-output-fit)-|(SCF-properties.south);
