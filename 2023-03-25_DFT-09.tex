...
\frame
{
	...
	平面波数目计算公式
	\begin{displaymath}
		N_{\mathrm{PW}}=\dfrac{4\pi}3\bigg(\sqrt{\dfrac{E_{\mathrm{cut}}}{\mathrm{RyToeV}}}\bigg)^3\times\dfrac{V_{\Omega}}{\mathrm{(a.u.To}\mbox{\textrm{\AA)}}^3}\times\dfrac1{8\pi^3}\times1.1
	\end{displaymath}
	{\tiny 其中\textrm{RyToeV}是单位换算常数\textrm{13.605826};~\textrm{a.u.To\AA}是单位换算常数\textrm{0.529177249}}
	\begin{itemize}
		\item \textrm{VASP}要求的基本(预保留)内存为\textrm{30M}
		\item 当体系规模较大时，波函数部分是内存消耗的最主要部分，其计算公式为\\
