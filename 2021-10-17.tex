...
\frame
{
	\frametitle{软件国产化的主要问题}
	以第一原理软件为例，开发国产软件的主要问题和制约因素
	\begin{itemize}
		...
		\item \textcolor{blue}{现有的考核评价体系限制}\\
			科学计算软件开发偏于工程技术问题且周期较长，与应用软件的科学研究成果回报比例差别大
	\end{itemize}
}
...
{
	\frametitle{}
	软件开发和运维是系统工程，但与普通商业软件不同，科学计算软件核心开发人员需要有较高的数理背景和较强的计算机编程和应用技能，而科学计算软件的服务范围明确、具体(目标市场小)
\begin{figure}[h!]
\vspace*{-0.10in}
\centering
\includegraphics[height=2.10in,width=3.75in,viewport=5 3 1250 780,clip]{Figures/Method_Procedure.png}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
...
\frame
{
	\frametitle{软件国产化的必要条件}
	\begin{itemize}
		\item \textcolor{magenta}{强有力的核心开发团队}\\
			队伍保持“少而精”，以第一原理计算软件为例，成员要求熟悉\textrm{DFT}软件的理论、算法和实现的程序细节，有较强的程序推导和软件实现能力
		\item \textcolor{magenta}{稳定充足的经费支持}\\
			科学计算软件的开发和维护成员的知识积累周期长、专业技术程度高，为保持开发队伍的稳定，必须有稳定的经费支持
		\item \textcolor{magenta}{规范的运维和管理}\\
			完整的科学计算软件代码量庞大，必须告别“作坊式”的开发-运维模式，对代码开发的日志、注释和版本实施全程管理
		\item \textcolor{magenta}{友好的\textrm{UI}界面}\\
			科学计算软件的功能和易用程度，决定了软件的可被替换程度，特别是服务实验为主的课题组时，软件的友好程度更是不可忽略的因素
	\end{itemize}
}
...
\frame
{
	...
	\begin{itemize}
		\item \textcolor{purple}{由全势(\textrm{Full Potential,~FP})方法出发}\\
			...
			\textcolor{brown}{策略}:~全面改写代码(\textrm{C/C++})，整合各模块，强化并行功能
...
\vspace*{-0.10in}
\centering
\includegraphics[height=1.00in,width=2.70in,clip]{Figures/Method_Relation.png}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
