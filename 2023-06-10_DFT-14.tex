...
{
	\frametitle{长波极限下的光学函数的计算}
	在$\vec k$空间计算纵向介电函数时，将遇到大量类似$\langle\psi_{\vec k+\vec q}^{\beta}|\mathrm{e}^{\mathrm{i}\vec q\cdot\vec r}|\psi_{\vec k}^{\alpha}\rangle$和$\langle\psi_{\vec k+\vec q}^{\beta}|\psi_{\vec k}^{\alpha}\rangle$的积分，处理策略是
	\begin{itemize}
		\item 将$\mathrm{e}^{\mathrm{i}\vec q\cdot\vec r}$对$\vec q$展开到一阶
			...
			\end{displaymath}
		\item 函数$|\psi_{\vec k+\vec q}^n\rangle$利用\textrm{Bloch}定理处理周期函数$|u_{\vec k+\vec q}^n\rangle$，同样对$\vec q$展开到一阶
			\begin{displaymath}
				|u_{\vec k+\vec q}^n\rangle=|u_{\vec k}^n\rangle+\vec q\nabla_{\vec k}|u_{\vec k}^n\rangle+\mathcal{O}(\vec q^2)
			\end{displaymath}
