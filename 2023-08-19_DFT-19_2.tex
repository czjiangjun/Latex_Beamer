...
\frame
{
	...
	\begin{itemize}
		\item \textrm{Mott}绝缘体:\\
			过渡金属氧化物\textrm{(Transition metal oxides)}\textrm{\ch{V2O3}}
		\item \textrm{Kondo}效应\\
			稀磁合金中出现的磁性杂质-非磁性宿主金属之间的对抗引起的低温反常输运行为
		\item 重\textrm{fermion}行为\\
			含有~$f$-电子的镧系或者锕系金属间化合物，有效质量比自由电子大$2\sim3$数量级
		\item 高温超导\textrm{(High-temperature Superconductivity)}\\
			钇钡铜氧\textrm{(\ch{YBa2Cu3O7-x})}、铁基超导体
	\end{itemize}
	电子强关联:~独立粒子图像失效，\textrm{DFT}和电子能带结构都无法描述
}
...
\begin{minipage}[b]{0.58\linewidth}
	\begin{displaymath}
		\begin{aligned}
			H=&-\sum_{\langle ij\rangle,\sigma=\uparrow,\downarrow}\underbrace{t_{ij}c_{i\sigma}^{\dag}c_{j\sigma}}_{\fontsize{6.2pt}{4.2pt}\selectfont{\textcolor{purple}{\mathrm{Kinetic~term}}}}\\
			&+\underbrace{\textcolor{blue}{U}n_{i\uparrow}n_{i\downarrow}}_{\fontsize{6.2pt}{4.2pt}\selectfont{\textcolor{purple}{\mathrm{Interaction~aterm}}}}~_{\fontsize{3.2pt}{2.2pt}\selectfont{\Longleftarrow\mathrm{(Coulomb)~U>0}}} \\
...
\vspace*{-25pt}
\begin{itemize}
	\item 半充满:~\textrm{I}~电子/格点平均:$\delta=0$
