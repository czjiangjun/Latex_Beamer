...
\frame
{
	...
	\begin{itemize}
		\item 时间序列的\textrm{Green's function}定义为$G(\vec x_1,t_1;\vec x_2,t_2)=-\mathrm{i}\langle\Theta_0^N|\hat{\mathbf{T}}[\hat\psi(\vec x_1,t_1)\hat\psi^{\dag}(\vec x_2,t_2)]|\Theta_0^N\rangle$
		\item \textrm{Green function}的\textcolor{blue}{\textrm{Lehmann}表象}为
			\begin{displaymath}
				\hspace*{-35pt}
				G(\vec x_1,\vec x_2;\omega)=\sum\limits_i\dfrac{\langle\Theta_0^N|\hat\psi_i(\vec x_1)|\Theta_0^{N+1}\rangle\langle\Theta_0^{N+1}|\hat\psi_i^{\dag}(\vec x_2)|\Theta_0^N\rangle}{\omega-\varepsilon_i+\mathrm{i}\eta\mathrm{sign}(\varepsilon_i-\mu)}+\mathrm{c.c.}\quad\eta\rightarrow0^+
			\end{displaymath}
			{\fontsize{6.2pt}{6.2pt}\selectfont{存在小的虚部$\eta$保证频率域的\textrm{Fourier}变换收敛}}

			这样表示的\textrm{Green's function}，在复平面中的极点与增加/失去一个粒子体系的能量对应，因此也称为\textcolor{blue}{谱表象\textrm{(Spectral~representation)}}
	\end{itemize}
\begin{figure}[h!]
\centering
\vspace{-5pt}
...
\frame
{
	...
	即使是单粒子\textrm{Green's function}也无法精确求解，必须作适当近似，一个合理的近似是写成\textrm{Kohn-Sham}本征态和本征值表示的\textrm{Lehmann}形式$G_{\mathrm{KS}}^0(\vec r_1,\vec r_2;\textcolor{brown}{\omega})$，于是\textrm{Dyson}方程可以写成
	\begin{displaymath}
		\hspace*{-10pt}
	...
	\end{displaymath}
	\begin{itemize}
		\item \textrm{Dyson}方程描述了相互作用体系$G$通过自能$\Sigma$与独立体系$G_0$(传播子)关联，自能$\Sigma$含时、非局域、非\textrm{Hermitian}的
%	\textrm{Dyson}方程
%	\begin{displaymath}
%		\begin{aligned}
