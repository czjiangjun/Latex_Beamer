...
\end{figure} 
\fontsize{8.5pt}{2.2pt}\selectfont{对有限体系，当外部电场$\vec E(\vec r)=0$，宏观电极化强度$\vec P$可用偶极矩$\vec d$表示}
\begin{displaymath}
...
\end{displaymath}
\textcolor{red}{电极化强度的变化$\Delta\vec P=\vec P^{(1)}-\vec P^{(0)}$只与电荷密度的改变$\Delta n=n_{\mathrm{tot}}^{(1)}-n_{\mathrm{tot}}^{(0)}$有关，而与电荷密度改变的途径无关}
}
