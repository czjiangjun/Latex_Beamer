...
\frame
{
	...
	\begin{itemize}
		...
		\item \textrm{HPC~(High Throughput Computing)}:
			\vskip 5pt
...
%	\textbf{HPC}
%	\begin{itemize}
%		\item \textrm{Focus:\,Large workflows of \underline{\textcolor{blue}{highly-interdependent}}}
...
%%-------------------------------------------------------------------------------------------------------------------------
%	\textrm{Parameter}&高性能计算\textrm{(HPC)}	&高通量计算\textrm{HTC}\\\hline
%\midrule        %% 绘制中间线
%%	\textrm{Stands for} &\textrm{HPC stands for High-Performance Computing} & \textrm{HTC stands for High Throughput Computing}\\\hline
%%	\textrm{Definition} &\textrm{HPC is defined as the type of computing that makes use of multiple computer processors in order to perform complex computations parallelly.} & \textrm{HTC is defined as a type of computing that parallelly executes a large number of simple and computationally independent tasks.}\\\hline
%%	\textrm{Workload} &\textrm{HPC consists of running large-scale, complex, and computationally intensive applications that need significant resources and memory. } &\textrm{HTC consists of running a large number of tasks that are independent and small in size and does not require a large amount of memory and resources.}\\\hline
%%	\textrm{Processing Power} &\textrm{HPC is designed to provide maximum performance and speed for large tasks.} &\textrm{HTC is designed to increase the number of tasks that needs to be completed in a given specific amount of time.}\\\hline
%%	\textrm{Resource Management} &\textrm{For resource management to processes, HPC makes use of job schedulers and resource managers. } &\textrm{For the resource management to processes, HTC makes use of distributed management resources.}\\\hline
%%	\textrm{Fault Tolerance} &\textrm{To reduce the risk of data loss and data corruption HPC systems have a complex fault tolerance mechanism.} &\textrm{HTC systems do not affect any other running processes due to the failure of an individual task.}\\\hline
%%	\textrm{Scaling} &\textrm{HPC scales up when few users are running together.} &\textrm{HTC systems scale horizontally for simple tasks and require less computational speed.}\\\hline
%%	\textrm{Applications} &\textrm{HPC can be used in applications such as engineering design, weather forecasting, drug discovery etc.} &\textrm{HTC can be used in applications such as bioinformatics, research applications, etc.}\\%\hline
%
%\end{tabular*}
%%\rule{\temptablewidth}{1pt}
%\bottomrule     %%绘制底部线
