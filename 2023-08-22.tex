...
\frame
{
	...
	\end{displaymath}
	$\mathbf{F}_{jj}(\mathbf{R})$一般比较小，可以忽略

	与\textrm{Ehrenfest-MD}中类似，如果取核波函数表示为极坐标形式
\begin{displaymath}
...
{\fontsize{6.2pt}{4.2pt}\selectfont{
	...
	\begin{displaymath}
		\begin{aligned}
			...
			&\textcolor{purple}{+\sum_{\gamma,i\neq j}\frac{\hbar}{M_{\gamma}}\frac{\mathbf{A}_j(\mathbf{R},t)}{\mathbf{A}_j(\mathbf{R},t)}\mathbf{d}_{ji}^{\gamma}(\mathbf{R})\cdot\nabla_{\gamma}\mathbf{S}_i(\mathbf{R},t)\times\Im\bigg[\mathrm{e}^{\mathrm{i}\phi_{ij}(\mathbf{R},t)}\bigg]}\\
...
{
	\frametitle{非绝热\textrm{Bohmian}动力学\textrm{(NABMD)}}
	引入动量$\nabla_{\beta}\mathbf{S}_j(\mathbf{R},t)|_{\mathbf{R(t)}}=\mathbf{P}_j^{\beta}(t)$，可有最终的核运动方程
	\begin{displaymath}
	...
	\end{displaymath}
	该方程描述了变量$\mathbf{R}(t)$的分量$\mathbf{R}_{\beta}$随时间的演化\\
	其中全微分定义为
	\begin{displaymath}
	...
	\begin{itemize}
		\item \textcolor{blue}{$\mathbf{Q}_j(\mathbf{R}(t),t)$}是量子势\\
			它描述了单一电子态下的量子效应，引入非局域属性
		\item \textcolor{purple}{$\sum_{ji}\mathbf{D}_{ji}(\mathbf{R}(t),t)$}是非绝热量子势\\
...
\frame
{
	\frametitle{\textrm{TSH}方法:~经典部分}
{\fontsize{8.2pt}{4.2pt}\selectfont{
	\textcolor{blue}{核体系演化过程的经典部分}构成构象空间$(\mathbf{R},t)$的轨迹\textrm{(trajectory)}

	在\textrm{TSH}方法中，$\mathbf{S}_j(\mathbf{R},t)$可以视为作用量，因此体系的轨迹由最小作用原理确定，即$\mathbf{S}(\mathbf{R},\mathbf{P},t)=0$，与经典力学的相空间描述类似，假设不同初始态对应的粒子演化轨迹彼此独立(轨迹独立假设，\textrm{ITA})，因此核波函数的密度可用穿过单位空间微元的轨迹数量的比例来计算，即
	\begin{displaymath}
	...
	\vskip 3pt
	\textcolor{blue}{注意:~这里给出的核经典轨迹由与非绝热耦合系数$\mathbf{d}_{ji}^{\gamma}(\mathbf{R}(t))$无关部分确定}，因此可有
	\begin{displaymath}
		\begin{aligned}
...
{
	\frametitle{\textrm{TSH}方法:~经典部分}
	上述方程确定了核运动的作用量经典部分$\mathbf{S}_j^{\mathrm{CL}}(\mathbf{R},t)$和振幅的经典部分$\mathbf{A}_j^{\mathrm{CL}}(\mathbf{R},t)$的演化过程，不难看出其与\textrm{Newton}运动方程等价
\begin{displaymath}
...
这种附件作用会使得经典概率密度$\bigg(A_j^{\mathrm{CL}}(\mathbf{R}(t),t)\bigg)^2$在电子态间传播(即体系可以在不同势能面之间跃迁)
}
...
{\fontsize{8.2pt}{4.2pt}\selectfont{
	...
	\begin{displaymath}
		\begin{aligned}
			...
			&\times\mathbf{d}_{ji}^{\gamma}(\mathbf{R})\cdot\dot{\mathbf{R}}_{\gamma}(t)\Im\bigg[\mathrm{e}^{\mathrm{i}\phi_{ij}^{\mathrm{QM}}(\mathbf{R},t)}\bigg]}\\
			...
			&\times\mathbf{d}_{ji}^{\gamma}(\mathbf{R})\cdot\dot{\mathbf{R}}_{\gamma}(\mathbf{R}(t))\Re\bigg[\mathrm{e}^{\mathrm{i}\phi_{ij}^{\mathrm{QM}}(\mathbf{R},t)}\bigg]}
		\end{aligned}
	\end{displaymath}
	其中
	\begin{displaymath}
...
在\textrm{TSH}动力学中，经典部分和量子部分的方程同时求解，并认为只要统计轨迹足够多，将有
\begin{displaymath}
...
\frame
{
	...
	\textrm{Tully}给出$t$时刻统计随后$\Delta t$时间内由$j$向$i$跃迁几率的表达式
			...
			\end{displaymath}
			实际计算中可以采用\textrm{Monte-Carlo}的思想来确定态间跃迁，比如生成随机数$\zeta\in[0,1]$，只有当满足
			\begin{displaymath}
