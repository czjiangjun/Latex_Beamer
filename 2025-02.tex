...
\begin{frame}{典型架构}
  ...
  \begin{tikzpicture}[node distance=1.8cm, auto]
    \node [rectangle, draw, fill=blue!20, text width=3cm, text centered] (phy) {物理世界\\传感器/设备};
    \node [rectangle, draw, fill=green!20, below of=phy] (data) {数据层\\\textrm{IoT/5G}传输};
    \node [rectangle, draw, fill=red!20, below of=data] (model) {数字模型\\仿真引擎/\textrm{AI}算法};
    \node [rectangle, draw, fill=yellow!20, below of=model] (app) {应用层\\预测/优化/决策};

    \path[->] (phy) edge (data)
              (data) edge (model)
              (model) edge (app)
              (app.east) edge[bend right=30] node[right] {反向控制} (phy.east);
  \end{tikzpicture}
\end{frame}
...
\subsection{典型应用}
\begin{frame}{智能制造}
	\begin{exampleblock}{\textrm{Siemens}工厂\textrm{in Amberg}}

\begin{figure}[h!]
%\vspace*{0.05in}
\centering
     \includegraphics[height=1.6in, width=2.9in, viewport=0 0 2650 1992,clip]{Figures/Siemens_Amberg-Factory.jpeg}
%\caption{\tiny \textrm{All three elements must be present for a true digital twin.}}%(与文献\cite{EPJB33-47_2003}图1对比)
