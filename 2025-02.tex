...
\section{什么是数字孪生？}
\begin{frame}{核心定义}
  \begin{block}{NASA定义（2010）}
    物理实体的高保真数字镜像，通过实时数据更新实现动态仿真
  \end{block}

  \begin{columns}
    \column{0.6\textwidth}
    \begin{itemize}
      \item 三要素：物理实体、数字模型、数据连接
      \item 关键特征：
      \begin{itemize}
        \item 虚实映射
        \item 实时交互
    ...
    \column{0.4\textwidth}
%    \includegraphics[width=\textwidth]{dt_basic.png}
  \end{columns}
\end{frame}
...
\begin{frame}{里程碑事件}
 ...
 %     \includegraphics[width=0.3\textwidth]{dt_timeline.png}
  ...
  \begin{itemize}
    \item 2002: 密歇根大学提出"镜像空间模型"
    \item 2010: NASA应用于航天器健康管理
    \item 2017: GE推出工业数字孪生平台
    \item 2021: 元宇宙概念推动数字孪生普及
  \end{itemize}
\end{frame}
...
\begin{frame}{典型架构}
  ...
  \begin{tikzpicture}[node distance=2cm, auto]
    \node (phy) [rectangle, draw, fill=blue!20, text width=3cm, text centered] {物理世界\\传感器/设备};
    \node (data) [rectangle, draw, fill=green!20, below of=phy] {数据层\\IoT/5G传输};
    \node (model) [rectangle, draw, fill=red!20, below of=data] {数字模型\\仿真引擎/AI算法};
    \node (app) [rectangle, draw, fill=yellow!20, below of=model] {应用层\\预测/优化/决策};

    \path[->] (phy) edge (data)
              (data) edge (model)
              (model) edge (app)
              (app) edge[bend right=30] node[right] {反向控制} (phy);
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{支撑技术}
  \begin{columns}
    \column{0.5\textwidth}
    \begin{itemize}
      \item \textcolor{blue}{物联网(IoT)}：实时数据采集
      \item \textcolor{green}{大数据分析}：数据价值挖掘
      \item \textcolor{red}{建模仿真}：ANSYS/Matlab
      \item \textcolor{purple}{AI算法}：数字模型优化
