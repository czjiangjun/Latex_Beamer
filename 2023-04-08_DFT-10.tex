...
\frame
{
	...
	\begin{displaymath}
		...
		\end{aligned}
	\end{displaymath}
	{\fontsize{6.2pt}{5.2pt}\selectfont{这里暂不考虑$\mathbf{H}$对$\mathbf{S}$的依赖}}\\
	\vskip 5pt
	\textrm{Hellmann-Feynman}定理指出:~如果\textrm{Hamiltonian}$\mathbf{H}$是\textrm{Hermitian}的，并且有
	\begin{displaymath}
...
\frame
{
	...
	基于\textrm{Born-Oppenheimer}近似的原子-电子耦合的势能面计算方案，每一原子步都需要完整的电子结构自洽迭代，计算量相当可观。
\vskip 5pt
\textrm{1985}年，在\textrm{Car-Parrinello}建议的方案中，\textcolor{purple}{电子态将原子核的运动一样，都用分子动力学算法计算}。\\
在该方案中，\textcolor{blue}{体系的电子态并未能达到当前正电荷环境的真实基态，但体系总能却与真实基态更为接近}\\
{\fontsize{6.2pt}{5.2pt}\selectfont{考虑\textcolor{blue}{体系总能}(即\textcolor{red}{电子态总能}+\textcolor{magenta}{原子核静电相互作用能})是电子态波函数$\psi_k$和原子核坐标$\mathbf{S}$的泛函
\begin{displaymath}
...
由波函数正交约束，体系的\textrm{Euler-Lagrange~}运动方程可表示为
	\begin{displaymath}
		...
		\end{aligned}
	\end{displaymath}
	{\fontsize{6.2pt}{5.2pt}\selectfont{\begin{itemize}
...
\frame
{
	\frametitle{能量泛函的直接优化:~电子态运动方程}
如果运动方程中引入阻尼项，则经过一段时间后，方程的解达到稳定态，前述随时间二阶导数项消失(等号左侧为零)
\begin{itemize}
	\item 电子态的运动方程与\textrm{Kohn-Sham}方程类似\\
		{\fontsize{6.2pt}{5.2pt}\selectfont{能量本征值由$\varepsilon_k$变成矩阵元$\Lambda_{kl}$}}
	\item \textrm{Lagrange}参数$\Lambda_{kl}$是时间相关的\\
		{\fontsize{6.2pt}{5.2pt}\selectfont{
		因此每个\textrm{MD}步必须重新计算$\Lambda_{kl}$，以确保电子态波函数满足正交约束条件}}
	\item \textcolor{purple}{电子近似基态可以通过求解\textrm{Verlet~}运动方程得到}
...
\frame
{
	\frametitle{能量泛函的直接优化:~原子核运动}
	原子核运动方程的求解主要围绕电子态总能对原子位置$\vec R_i$的求导，虚考虑求导贡献的共三部分
	\begin{itemize}
	...
	\end{itemize}
	\textrm{Car-Parrinello HF}方法得到的结果与二体势(力场)方法结果等价
	\vskip 5pt
...
\frame
{
	...
	\end{displaymath}}
	\textrm{Langrage}乘子确保电子轨道波函数正交%，可有
...
%	$X_{ij}$要求关于$\mathbf{X}$的矩阵方程
	...
	\textcolor{purple}{不同的旋转方式会对\textrm{Verlet}算法的执行效率产生很大的影响}
%	\begin{displaymath}
