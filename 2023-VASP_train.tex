%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  不使用 authblk 包制作标题  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-------------------------------PPT Title-------------------------------------
\title{{\rm VASP}算例举要\footnote{\fontsize{6.2pt}{5.2pt}\selectfont{本讲义算例主要来自文献\cite{J.-G._Lee}。因\textrm{VASP}版本的不同，及用户用于计算的\textrm{POTCAR}数据差异，所以重复讲义中算例时，计算过程和结果的数据，绝对数值可能会有一点出入，但应该不会差别太大。}}}
%-----------------------------------------------------------------------------
%----------------------------Author & Date------------------------------------
...
\textrm{OSZICAR}存储的是\textrm{VASP}迭代循环过程中的能量变化情况%，部分内容如图\ref{Pt_atom:OSZICAR}所示。
\begin{figure}[h!]
\centering
...
\end{figure}
{\fontsize{7.0pt}{5.2pt}\selectfont{最后一行$\mathrm{E}_0$的值($-0.522\mathrm{eV}$)就是当前参数设置下，单个\textrm{Pt}原子在真空中的基态能量}}
}
...
\begin{itemize}
	\item $\mathrm{E}_0$($-.522\mathrm{eV}$)不是原子的绝对能量，而是\textrm{Pt}原子相对于\textrm{PAW}势的能量\\
		\textrm{Pt}的原子构型为([\ch{Xe}]$4\mathit{f}^{14}5\mathit{d}^96\mathit{s}^1$)
	\item 自旋极化计算得到\textrm{Pt}原子磁矩为$\mathrm{mag}=1.9986\mu_{\mathrm{B}}$(\textrm{Bohr magneton})
\begin{displaymath}
...
\end{figure}
{\fontsize{7.0pt}{5.2pt}\selectfont{计算中不再要求输出电荷密度和波函数到\textrm{CHGCAR}和\textrm{WAVECAR}文件，计算控制参数中引入\textit{ISYM}=1}}
...
本部分我们将学习如何利用\textrm{VASP}计算\ch{SiC}的光学介电函数，关于\textrm{VASP}中光学介电函数计算主要参见文献\upcite{PRB73-045112_2006}。
