...
\frame
{
	...
	\textrm{WIEN2k}软件在旋-轨耦合计算时，须指定磁化轴向。从群论的角度考虑，旋-轨耦合效应会引入双值群\textrm{(Double group)}\footnote{\fontsize{5.2pt}{6.2pt}\selectfont{轨道角动量是三维空间的连续旋转本征值，可用\textrm{SO(3)}群表示;~自旋角动量则是二维复矢量的幺正群，用\textrm{SU(2)}群表示。两个群的\textrm{Lie}代数同构，但\textrm{Lie}群群元是一对二的关系。而点群都是\textrm{SO(3)}群的子群，因此不能用来描述\textrm{SU(2)}群，从而引入双值群}}
\begin{figure}[h!]
\centering
\vspace*{-0.21in}
\hspace*{-0.1in}
\includegraphics[height=1.8in,width=2.1in,viewport=2 5 1162 1180,clip]{Figures/euler-angles-yaw-aircraft-principal-axes-orientation-cartesian-coordinate-system.png}
